...
当用户需求不完整、目标不清晰、边界模糊，或用户明确要求"帮我梳理""拷问我""grill me""压测方案"时，先不要生成文档。

通过一问一答澄清需求，直到足以生成稳定的 `prd.md`：

1. 每次只问一个问题，避免一次性抛出长问卷
2. 每个问题都提供推荐答案，帮助用户快速决策
3. 沿决策树追问，优先解决会影响后续文档结构的关键决策
4. 先解决上游依赖问题，再进入下游细节，不跳跃追问
5. 如果问题可以通过读取当前代码库、README、已有 `docs/` 或配置文件回答，则先查看项目内容，不向用户重复提问
6. 核心决策明确后，先总结已确认内容，再进入阶段 1
...
- P0 / P1 / P2 优先级
- 明确不做的范围
- 是否涉及前端、后端、数据库、权限、支付、运维或测试
- 兼容性、迁移、风险、合规要求

> 阶段 0 是按需启用：简单明确的需求可以直接进入阶段 1；复杂或模糊需求必须先澄清。
...
用户：我想加个登录功能
AI：
  → 判断需求过于模糊，进入阶段 0：需求澄清与方案拷问
  → 追问："登录方式是什么？（推荐：账号密码 + 手机验证码，先保证基础闭环）"
  → 追问："这是新版本还是在现有版本上迭代？（推荐：如果已有 PRD，作为 minor 版本迭代）"
  → 追问："是否需要安全规则？（推荐：首次登录强制改密、异常登录提醒列为 P1）"
  → 用户回答后
  → 总结已确认的用户角色、核心流程、优先级、非目标
  → 继续正常流程，生成 prd.md
```

#### 4. 用户要求 grill me / 拷问方案

```
用户：先 grill me，帮我把会员订阅方案问清楚
AI：
  → 进入阶段 0，不直接生成文档
  → 逐个追问：套餐类型、试用规则、续费、退款、权益边界、支付失败处理
  → 每个问题都给推荐答案
  → 如果当前项目已有会员/支付相关代码或文档，先读取后再问
  → 关键分支确认后，总结决策树
  → 询问是否基于总结生成 prd.md
```
